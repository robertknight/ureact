...
function outputKey(output: ChildOutput) {
  return isComponent(output) ? vnodeKey(output.vnode) : null;
}
  ...
  _diffChildren(component: Component, vnode: VNode, el: Element) {
    ...
    if (vnode.props.children) {
      ...
      for (let child of flattenChildren(vnode.props.children)) {
        ...
        if (childKey !== null) {
          prevComponent = prevOutput.find((o) => outputKey(o) === childKey);
        } else {
          ++nonKeyedCount;
          let nonKeyedIndex = -1;
          prevComponent = prevOutput.find(
            (o) => outputKey(o) === null && ++nonKeyedIndex === nonKeyedCount
          );
        }
